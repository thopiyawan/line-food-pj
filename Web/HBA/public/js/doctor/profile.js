...
//เรียกค่าเฉลี่ยทั้งหมดของสัดส่วนที่กิน
async function getAverageResult() {
    ...
    try {
        ...
        //Chart ของแต่ละหมวด โปรตีน คาร์โบ ไขมัน
...
//ตาราง Kcal เปอร์เซ็นแต่ละหมวด โปรตีน คาร์โบ ไขมัน
        ...
        querySnapshot.forEach(function (doc) {
            ...
            db.collection("users").doc(userId).collection('diary').doc(dateKcal).collection('history').doc('total').get().then(function (doc) {
                ...
                tr.find('#btnHistory').click(function () {
                    window.open(`/history?userId=${userId}&date=${dateKcal}`, "_self");
                })
            })
    ...
    db.collection("users").doc(userId).collection("diary").orderBy("timestamp", "desc").get().then(function (querySnapshot) {
        querySnapshot.forEach(function (doc) {
            ...
            db.collection("users").doc(userId).collection('diary').doc(dateRatio).collection('history').doc('total').get().then(function (doc) {
                ...
                tr.find('#btnHistory').click(function () {
                    window.open(`/history?userId=${userId}&date=${dateRatio}`, "_self");
                })
            })
...
`
//เรียกประวัติคำแนะนำจากผู้เชี่ยวชาญ
function getComment() {
...
async function getHistoryWeight() {
    var weight = []
    try {
        await db.collection("users").doc(userId).collection('heigthweigth-record').orderBy("timestamp", "asc").get().then(function (querySnapshot) {
            querySnapshot.forEach(function (doc) {
                const obj = {
                    "date": doc.id,
