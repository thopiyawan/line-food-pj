...
$('#linkToProfile').click(function () {
    window.open(`/profile?userId=${userId}`, "_self");
})
$('#linkToPatient').click(function () {
    window.open(`/patient`, "_self");
})
//วันที่เรียกดูย้อนหลัง
...
//เรียกข้อมูลทั้งหมดย้อนหลัง 7วัน
function getData() {
...
//คำนวณค่าเฉลี่ยของอาหารทั้งหมด
    ...
    //ส่งคำแนะนำไปยังผู้ใช้แบบคร่าว ๆ
    btnSendComment.addEventListener('click', e => {
        const date = moment().format('DD-MM-YYYY');

        var comment = 'พลังงานที่ได้รับ'
        if (resultBmr == 'เหมาะสมกับพลังงานที่ควรได้รับต่อวัน') {
            comment += 'เหมาะสมกับที่ควรได้รับ'
        } else if (resultBmr == 'มากกว่า') {
            comment += 'เกินกว่าที่ควรได้รับ แนะนำให้ลดปริมาณลงเพื่อสุขภาพที่ดียิ่งขึ้น'
        } if (resultBmr == 'น้อยกว่า') {
            comment += 'น้อยกว่ากว่าที่ควรได้รับ แนะนำให้เพิ่มปริมาณลงเพื่อสุขภาพที่ดียิ่งขึ้น'
        }
        db.collection("users").doc(userId).collection('comment').doc(date).set({
...
//Export ข้อมูลจากตารางออกไปเป็นไฟล์ Excel
...
btnSlectDate.addEventListener('click', e => {
    ...
    window.open(`/record?userId=${userId}&dateStart=${dateStart}&dateEnd=${dateEnd}`, "_self");
})