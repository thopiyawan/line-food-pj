async function calculateBalance(userId) {
    ...
    try {
        var getRatio
        // Get ratio วันล่าสุดที่กำหนด
        await db.collection("users").doc(userId).collection("ratio-record").orderBy("timestamp", "desc").limit(1)
            .get()
        ...
        // ดึงข้อมูลอาหารทั้งหมดที่บันทึกในวันนี้
        ...
        // บันทึกสัดส่วนตั้งต้น
        await db.collection("users").doc(userId).collection("diary").doc(date).collection("history").doc("ratio").set(
            getRatio
        )
